...
namespace MidiExplorer.GuitarPro
{
    public class NativeNote
    {
        ...
        public NativeNote(NativeNote old)
        {
    ...
    public class NativeTrack
    {
        ...
        public List<NativeNote> notes = new List<NativeNote>();
        ...
        public MidiExport.MidiTrack getMidi()
        {
            ...
            int currentIndex = 0;
            NativeNote _temp = new NativeNote();
            ...
            foreach (NativeNote n in notes)
            {
               ...
               new string[] { "" + ch, "7", "" + vc[1] }, vc[0] - currentIndex));
                ...
                if (n.str - 1 < 0) Log.Debug("String was -1");
                if (n.str - 1 >= tuning.Length && tuning.Length != 0) Log.Debug("String was higher than string amount (" + n.str + ")");
        ...
        private List<NativeNote> addSlidesToNotes(List<NativeNote> notes)
        {
            List<NativeNote> ret = new List<NativeNote>();
            int index = -1;
            foreach (NativeNote n in notes)
            {
                index++;
                ...
                if ((n.slideInFromBelow && n.str > 1) || n.slideInFromAbove)
                {
                    ...
                    {
                        NativeNote newOne = new NativeNote(n);
    ...
    public class NativeFormat
    {
        ...
        public List<NativeTrack> tracks = new List<NativeTrack>();
        ...
        public MidiExport.MidiExport toMidi()
        {
            ...
            foreach (NativeTrack track in tracks)
            {
        ...
        public List<NativeTrack> retrieveTracks(GPFile file)
        {
            List<NativeTrack> tracks = new List<NativeTrack>();
        ...
        public void addToTremoloBarList(int index, int duration, BendEffect bend, NativeTrack myTrack)
        {
            int at;
        ...
        public List<NativeNote> retrieveNotes(global::Track track, int[] tuning, NativeTrack myTrack)
        {

            List<NativeNote> notes = new List<NativeNote>();
            int index = 0;
            NativeNote[] last_notes = new NativeNote[10];
            ...
            foreach (Measure m in track.measures)
            {
                ...
                if (m.simileMark == SimileMark.simple) //Repeat last measure
                {
                    ...
                    {
                        NativeNote newNote = new NativeNote(notes[x]);
                ...
                foreach (Voice v in m.voices)
                {
                    ...
                    foreach (Beat b in v.beats)
                    {
                        ...
                        {
                            NativeNote note = new NativeNote();
                            //Beat values
                            ...
                            if (n.effect.tremoloPicking != null || n.effect.trill != null)
                            {
                                ...
                                {
                                    NativeNote newOne = new NativeNote(note);
                            ...
                            if (n.effect.grace != null)
                            {
                                ...
                                { //GP3,4,5 format
                                    ...
                                    NativeNote graceNote = new NativeNote();
                        ...
                        if (hasBrush && brushDirection == BeatStrokeDirection.up)
                        {
                            ...
                            int notesCnt = b.notes.Count;
                            NativeNote[] temp = new NativeNote[notesCnt];
                            for (int x = notes.Count - notesCnt; x < notes.Count; x++)
                            {
                                temp[x - (notes.Count - notesCnt)] = new NativeNote(notes[x]);
                            }
                            for (int x = notes.Count - notesCnt; x < notes.Count; x++)
                            {
